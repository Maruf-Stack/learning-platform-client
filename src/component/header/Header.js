import React from 'react';
import { useContext } from 'react';
import { Link } from 'react-router-dom';
import { AuthContext } from '../../context/AuthProvider';
import { IconName, FaUserCircle } from "react-icons/fa";
import './header.css'
const Header = () => {
    const { user, singOut } = useContext(AuthContext)
    console.log(user)
    const handlesingOut = () => {
        singOut()
    }
    const menu = <>
        <li><Link to="/" className='mx-4'>Home</Link></li>
        <li> <Link to="/courses" className='mx-4'>Courses</Link></li>
        <li><Link className='mx-4' to='/faq'>FAQ</Link></li>
        <li><Link to="/blog" className='mx-4'>Blog</Link></li>
        {
            user?.uid ?
                <Link onClick={handlesingOut} to="/" className='mx-4'><button className="btn btn-outline px-9 btn-error">Logout</button></Link> :
                <Link to="/login" className='mx-4'><button className="mx-4 btn btn-outline btn-info px-9 lg:mx-0">Login</button></Link>
        }
    </>
    return (
        <div className='header'>
            <div className='items-center justify-around lg:mb-5 lg:flex'>
                <div className='flex items-center h-auto title'>
                    <img src="https://media.istockphoto.com/vectors/online-education-concept-line-icons-internet-learning-courses-distant-vector-id914519732?k=20&m=914519732&s=612x612&w=0&h=L_kl7uysq7s8m2V_nqQJye7s9oTxUUXoPV8McpSPRzw=" alt="" className='w-24' />
                    <p className='text-3xl font-bold'><span className='e'>E</span>xpert in Technology</p>
                </div>


                <div className='font-semibold nav font-[popins] mb-10'>
                    <div className="navbar-start">
                        <div className="dropdown">
                            <label htmlFor='menu' tabIndex={0} className="btn btn-ghost lg:hidden">
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                            </label>
                            <ul tabIndex={0} className="p-2 mt-3 shadow menu menu-compact dropdown-content bg-base-100 rounded-box w-52 smmenu">
                                {
                                    menu
                                }
                            </ul>

                        </div>
                    </div>
                    <div className="hidden navbar-center lg:flex">
                        <ul className="p-0 font-semibold menu menu-horizontal lgmenu">
                            {
                                menu
                            }
                        </ul>
                        <label className=" swap swap-rotate">
                            <input type="checkbox" className='mt-5' />


                            <svg className="w-10 h-10 fill-current swap-on" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" /></svg>
                            <svg className="w-10 h-10 fill-current swap-off" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" /></svg>

                        </label>
                    </div>





                </div>
            </div>
            <div className='mb-5 ml-10 lg:ml-40'>
                <p className='mt-4 font-sans font-bold text-blue-800'>Username : {user?.displayName}</p>
                {
                    user?.photoURL ? <div className='tooltip tooltip-info tooltip-left' data-tip={user?.displayName}>
                        <img className='w-20 h-20 mt-2 font-sans rounded-full' src={user?.photoURL} alt='avatar'></img>
                    </div>
                        : <FaUserCircle></FaUserCircle>}
            </div>
        </div>
    );
};

export default Header;